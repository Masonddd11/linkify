...
export function ClientHero({ session, slug }: { session: Session; slug: string }) {
  return (
    <section className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="text-center px-4 max-w-3xl mx-auto relative z-10">
        ...
        {/* ... existing motion divs and content ... */}
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        ...
        >
          ...
          >
            <Link href={session ? `/${slug}` : "/signup"}>
              {session ? "Back to Your Page" : "Create Your Page"}
            </Link>
          </Button>
          <Button
            asChild
